    ...
    getChat(){
        axios.get('/api/chat/writer/get').then(res => {
            if(res.status === 200){
                ...
                if (this.props.id) {
					axios.get('/api/student/get/single/'+this.props.id).then(res => {
						if(res.status === 200){
					...
					this.getMessage(res.data['0'].student_id, res.data['0'].name)
				}
            }
		});
    }
				...
				<div className="sidebar-content">
					...
					{/* <!-- Sidebar search --> */}
						<div className="panel panel-flat">
                         ...
                         return(
										<li key={i}><a onClick={this.getMessage.bind(this, c.student_id , c.name)} className="a-active">{c.name}<span style={{float:"right", background: "#90ab00", padding:4 , borderRadius: 15, color: "#FFF", textAlign: "center"}}>{this.state.count}</span></a></li>
                         )})}
									</ul>
								</div>
							</div>
